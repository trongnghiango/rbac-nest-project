...
* **Bắt buộc:** Để **ghi/cập nhật** dữ liệu của Module khác, phải sử dụng `EventBus` (Choreography) để tránh Circular Dependency. Các module tự nghe Event và tự xử lý data của mình.
...
| **Accounting** | Tier 3 | Kế toán (Thu/Chi) | Sử dụng Value Object (`Money`). Tự động sinh PDF. Target Strategy. |
