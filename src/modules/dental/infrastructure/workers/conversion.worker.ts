...
async function convertAndEncrypt(task: ConversionTask): Promise<WorkerResult> {
  ...
  try {
    ...
    const gltfTransform = path.join(
      binPath,
      isWin ? 'gltf-transform.cmd' : 'gltf-transform',
    );
    const gltfPipeline = path.join(
      binPath,
      isWin ? 'gltf-pipeline.cmd' : 'gltf-pipeline',
    );

    // Pipeline xử lý 3D
    await execFilePromise(
      obj2gltf,
      ['-i', objFilePath, '-o', initialGlb, '--binary'],
    ...
    await execFilePromise(
      gltfPipeline,
      ['-i', simplifiedGlb, '-o', optimizedGlb, '--draco.compressionLevel=10'],
      { timeout: config.timeout },
    );
